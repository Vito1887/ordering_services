...
function setupCaptcha(form) {
  ...
  if (!checkbox || !details || !textEl || !inputEl || !refreshBtn || !errorEl)
    return;
  ...
  function validateBeforeSubmit() {
    ...
    if ((hasName || hasEmail || hasTel) && !checkbox.checked) {
      errorEl.textContent =
        "Для отправки необходимо пройти проверку на робота.";
      return false;
    }
    return true;
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll(".modules-order-btn").forEach((button) => {
    button.addEventListener("click", function () {
      ...
      const title =
        item?.querySelector(".modules-item-title")?.textContent?.trim() ||
        "Услуга 1С";
      openModal(title);
    });
  });
  ...
  // Обработка кнопок "Оставить заявку" на странице report.html
  document
    .querySelectorAll(
      ".report-order-btn, .report-apply-btn, .report-tariff-btn",
    )
    .forEach((button) => {
      button.addEventListener("click", function () {
        openModal("СФР бесплатно");
      });
    });

  // Обработка кнопки "Откликнуться на вакансию"
  document.querySelectorAll(".vacancy-apply-btn").forEach((button) => {
    button.addEventListener("click", function () {
      openModal("Вакансия");
    });
  });
